...
StopAddonSpam.ruleset = {
	...
	rules = {
		...
		["rbuffframestyler"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("rbuffframestyler")) end,
			action = DENY
		},
		["rbuffframestyler2"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("/rbfs")) end,
			action = DENY
		},
		["askmrrobot"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("loaded ask mr")) end,
			action = DENY
		},
		["itemleveldisplay"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("itemleveldisplay")) end,
			action = DENY
		},
		["questuntracker"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("quest un")) end,
			action = DENY
		},
		["timelessanswers"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("ta")) end,
			action = DENY
		},
		["auctionator"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("<auctionator>")) end,
			action = DENY
		},
		["leatrix"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("leatrix plus")) end,
			action = DENY
		},
		["bagsync"] = {
		...
		["ifthen"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("ifthen")) end,
			action = DENY
		},
		["mounty"] = {
			test = function (msg, id, frame) return string.find(string.lower(msg), string.lower("<mounty>")) end,
			action = DENY
		},
		["default"] = {
			action = ALLOW
